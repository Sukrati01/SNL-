...
const getCurrentActivePiece = () => {
  ...
  if (turn === 0) currPiece = document.querySelector(".piece1");
  else currPiece = document.querySelector(".piece2");
  return currPiece;
};
...
const getBlockIDAtCurrActivePiece = () => {
  ...
  if (rowNum % 2 == 0) return 100 - rowNum * 10 - colNum;
  else return 100 - rowNum * 10 - (10 - colNum - 1);
};
...
const getDirection = () => {
  ...
  if ((x === startCentreX || x === -startCentreX) && y === maxCoord)
    // ID  == '1'
    direction = "right";
  else {
    let rowEvenOdd = undefined; // 0 for evenEnding and 1 for oddEnding
    if (((y - startCentreX) / centreOffSet) % 2 != 0)
      // at evenEnding
      rowEvenOdd = 0;
    else rowEvenOdd = 1; // at oddEnding

    if (rowEvenOdd === 0) {
      if (x === maxCoord) direction = "up";
      else direction = "right";
    } else {
      if (x === startCentreX) direction = "up";
      else direction = "left";
    }
  }
...
const movePiece = (direction) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (direction === "up")
      ...
      currPiece.style.left = `${
        parseInt(currPiece.style.left) + centreOffSet
      }px`;
...
const rollAction = (dieValue) => {
  return new Promise(async (resolve, reject) => {
    if (getBlockIDAtCurrActivePiece() + dieValue > 100) resolve();
    else {
    ...
    if (getBlockIDAtCurrActivePiece() === 100) {
      ...
      const winnerDeclarationText = document.querySelector(
      ...
      if (turn === 0)
        ...
        winnerUserName = document.querySelector(
          ".second-player-name"
        ).innerHTML;
...
const addRollClickEventListener = (targetBtnClass, dieImg, mainTurn) => {
  ...
  targetBtn.addEventListener("click", async (e) => {
    if (!stopRollCallback) {
      if (
        (turn === 0 && targetBtnClass === ".roll-die-btn") ||
